...
houston_before_plot <- tm_shape(houston_before) +
  tm_raster(col.scale = tm_scale_continuous(values = viridis(9))) +
  tm_layout(legend.show = FALSE,
            title.size = 0.7) +
  tm_title("Houston Light Intensity Before Storm (Feb. 7th)") +
...
houston_after_plot <- tm_shape(houston_after) +
  ...
  tm_layout(legend.show = FALSE,
            title.size = 0.7) +
  tm_title("Houston Light Intensity After Storm (Feb. 16th)") +
  tm_compass(position = c("right", "bottom"), size = 1) +
  tm_scalebar(position = c("left", "bottom"))
...
```{r}
# Add dummy column to make legend 
homes_in_blackout$category <- "Blackout Area"
...
homes_in_blackout_plot <- tm_shape(homes_in_blackout) +
  tm_polygons(
    fill = "category",      # Use the category variable
    fill.scale = tm_scale_categorical(values = "black"),  
    fill.legend = tm_legend(title = "Affected Buildings"),
    col = "black",
    lwd = 0.5
  ) +
...
```{r}
# Add dummy column to make legend 
tract_in_blackout$status <- "Affected Census Tracts"

# Plot census tracts in blackout zone
tract_in_blackout_plot <- tm_shape(tract_in_blackout) +
  tm_polygons(
    fill = "status",
    fill.scale = tm_scale_categorical(values = "gray70"),
    fill.legend = tm_legend(title = "Blackout Areas"),
    col = "black",
    lwd = 0.5
  ) +
